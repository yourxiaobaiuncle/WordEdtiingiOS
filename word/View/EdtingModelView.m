...
-(void)getString:(NSNotification*)sender{
    ...
    self.fontFimaly=[dict objectForKey:@"fontFimaly"];
...
-(void)creatModel1ViewwithLabelAuthorText:(NSString*)authorString{
    ...
    if ([authorString isEqualToString:@" 作者"]||authorString.length==0) {
        sv=[[SView alloc] initWithFrame:CGRectMake(20*XHSizeW, 150*XHSizeH, 28*XHSizeW, 142*XHSizeH) andString:@" 作者" andTextFont:12.0f andTextColor:[UIColor lightGrayColor] andWordStyle:vertical andFontFimaly:self.fontFimaly];
        [sv.layer addSublayer:[WordTool creatPointLine:sv]];
    }else{
...
-(void)creatModel2ViewwithAoutherLabel:(NSString*)aoutherString{
    ...
    if ([aoutherString isEqualToString:@" 作者"]||aoutherString.length==0) {
        ...
        sv=[[SView alloc] initWithFrame:CGRectMake(20*XHSizeW, 150*XHSizeH, 28*XHSizeW, 142*XHSizeH) andString:aoutherString andTextFont:12.0f andTextColor:[UIColor colorWithHex:@"#3C3C3C" andColorAlpha:1.0] andWordStyle:vertical andFontFimaly:self.fontFimaly];
    }
    [self addSubview:sv];
    sv.backgroundColor=[UIColor clearColor];
    UITapGestureRecognizer *tapauthor=[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tapGesTextClick:)];
